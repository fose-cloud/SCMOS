...
A site the destination calls by another word is read through `CorrectionRules.SiteSynonyms` (`LADKRABANG` / `LAD KRABANG` = `LKB`, `KABINBURI` = `KABIN BURI`, `HAZHEM` = `HAZCHEM`), added only when a report showed the pair; TYPE is evidence too (`TK` / ISO tank = the rotation's `: TANK` account — ALTEK); when one rotation row is the plain name and the others only add a qualifier the job does not show, the plain name is the job's (`ALTEK` on a 1X40' → `ALTEK INTERNATIONAL`); a site named in part counts when it is the only candidate any word of the destination belongs to (`TROY` to `HAZCHEM → TROY (HAZCHEM K.39)`, `DANA` to `SAHA AUTOPART → DANA (SAHA AUTO)`), a word of under three letters or a bare number never being evidence; the report prints, for every bare name with several sites, what the jobs' DESTINATION | PLANT LOADING actually say, so the next synonym is a fact and not a guess.
