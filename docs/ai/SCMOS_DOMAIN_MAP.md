...
| Dashboard / management briefing | `Dashboard.tsx`, `ControlTower.tsx`, `Today.tsx`, `BriefingBand.tsx` | `/api/dashboard/today`, `/rates`, `/briefing`, `/api/notifications`; `DashboardService`, `MonitorService`, `NotificationService` | Reads current operational tables/cache; no standalone AI metric table |
