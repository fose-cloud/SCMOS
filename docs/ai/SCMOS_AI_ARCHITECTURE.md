...
| `AI:SreAgentEnabled` | `AI__SreAgentEnabled` | false — the SRE Agent (Phase 7): the platform's own health (process, database answer time, register cache, configuration, workers' last signs of life), GitHub workflow runs, failure counts by kind; safe identifiers only; nothing restarted, rolled back or changed |
...
- `GET /api/ai/status`: non-secret configuration booleans and caller-authorized specialist metadata. ProviderConfigured is configuration-only, not a live-provider test. Enabled, non-mock status probes the mapped audit shape with the audit's write budget (half of `AI:TimeoutSeconds`, 5–30 s; a flat five seconds until 21 Sep 2026, when the working-hours database could not commit a run's first event inside it). Disabled/mock status never queries SQL. AuditReady means the read probe succeeded, not a guarantee of future writes; every run still commits its audit before execution. WriteToolsReady always remains false.
