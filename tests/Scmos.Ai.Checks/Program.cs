...
Check(agents.All.Count == 9 && agents.Resolve(request)?.Id == operations.Id, "nine specialists (the Communication Agent since Phase 4, the Document & Invoice Agent in the billing descriptor's place since Phase 5) and deterministic master default");
...
await DocumentChecks.RunAsync(Check);
